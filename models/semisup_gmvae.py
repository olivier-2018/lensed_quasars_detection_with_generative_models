...
class SEMISUP_GMVAE(BaseVAE):
    ...
    def __init__(self,
                 ...
                 debug:int = 0,
                 **kwargs) -> None:
        super(SEMISUP_GMVAE, self).__init__()
    ...
    def forward(self, sample):
        ...
        # qy = torch.stack((torch.logical_not(labels)*1, labels,), dim=1)
        # qy_logit = torch.logit(qy, eps=1E-6)
        ...
        for i in range(k):
            ...
            # Evaluate respective cluster latent space variable from conditional proba of z given x,y
            z[i], zm[i], zv[i] = self.qz_xy(x, y)                   # all [B, Ldims] 
            # Evaluate respective cluster latent space priors
    ...
    def loss_function(self, *args, **kwargs) -> dict:
        ...
        generative process:
            ...
            p_theta(x, y, z) = p(y) p_theta(z|y) p_theta(x|z)     
        with
            y ~ Cat(y|1/k)
        ...
        ELBO = + Eq_phi(z|x,y) [log p_theta(x|z)]    # reconstruction
                -KL(q_phi(z|x, y) || p_theta(z|y))   # KLD between variational posterior q(z|c,x) and GM priors p(z|x,c) 
                    - KL(q_phi(y|x) || p(y))         # KLD between conditional latent class variable proba and its marginal proba
        ...
        # loss_qy = torch.sum(qy * torch.nn.LogSoftmax(1)(qy_logit), 1)  # Neg entropy from logit
                
        loss_supervised = F.binary_cross_entropy(qy[:,0], labels.float(), reduction="none")
        
        # loss per cluster (with KLD per latent dimention)
        ########
        losses_i = []        
        recon_loss_i = []
        kld_loss_i = []
        for i in range(self.k):
            rec_loss = torch.sum(F.mse_loss(input, px[i], reduction='none'), dim=(1,2,3))           # [B] summed over all pixels
            # rec_loss = self.BCELogits(input, px[i], eps=1E-8)                                     # [B] BCE uses probas DOES NOT WORK
            log_norm = self.log_normal(z[i], zm[i], torch.exp(zv[i]), axis=None)                    # [B, Ldims] logpx
            log_norm_prior = self.log_normal(z[i], zm_prior[i], torch.exp(zv_prior[i]), axis=None)  # [B, Ldims] logpz
        ...
        loss = torch.stack([loss_supervised] + [loss_qy] + [qy[:, i] * losses_i[i] for i in range(self.k)]).sum(0)  # [B]    


        # loss per cluster (with summed KLD over latent space)
        ########
        # losses_i = []
        # for i in range(self.k):
        #     losses_i.append(
        #         self._loss_per_class(
        #             input, px[i], z[i], zm[i], torch.exp(zv[i]), zm_prior[i], torch.exp(zv_prior[i]))
        #         )
        # loss = torch.stack([loss_supervised] + [loss_qy] + [qy[:, i] * losses_i[i] for i in range(self.k)]).sum(0)
        # recons_loss = torch.zeros_like(loss)
        ...
        scalar_outputs = {"full_loss": tensor2float(loss.mean()),
                          ...
                          "KL_loss": tensor2float(loss_supervised.mean()), #.sum(1)
    ...
    def _loss_per_class(self, x, x_hat, z, zm, zv, zm_prior, zv_prior):
        # loss_px_i = self.BCELogits(x, x_hat)
        loss_px_i = torch.sum(F.mse_loss(x, x_hat, reduction='none'), dim=(1,2,3))
    ...
    def BCELogits(self, x, px_logits, eps=1E-8):
        ...
        return loss.view(batch_size, -1).sum(axis=1)
